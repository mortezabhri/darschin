...
export default memo(({ title, description, badgeID, lessonID }) => {
       ...
       return (
              <>
                     <Modal onClose={() => setOpen(prev => !prev)} isOpen={open} state={2} textTitle={"شرح برچسب"}>
                            <h1 className="pt-4 pb-4 w-full text-xl text-center font-iranisans border-b" dir="rtl">{title}</h1>
                            <p className="pt-6 text-center w-full font-iranisans" dir="rtl">{description ?? "\"بدون توضیحات\""}</p>
                            <div className={`absolute top-3 left-3  bg-neutral-300 text-neutral-800 rounded-xl p-0.5 flex transition-all cursor-pointer h-9 ${!clicked ? "min-w-9 max-w-9" : "min-w-56 max-w-56"} overflow-hidden`} onClick={() => deleteBadge(lessonID, badgeID)}>
                                   ...
                                   <div className="flex justify-center items-center text-sm font-iranisans min-w-44 max-w-44">
                     ...
                     <span
                            ...
                            className="w-[90%] px-0.5 py-0.5 bg-gray-600 text-white rounded-full text-xs text-center"
                     >
                            {title}
                     </span>
              </>
